...
source node-network-filter/env.sh
...
envsubst < config.yaml.template > /tmp/node-network-filter-config.yaml

checks

# Run Kraken
cat $SCENARIO_FOLDER/network-filter.yml
cat /tmp/node-network-filter-config.yaml

[ -z "$JUNIT_TESTCASE" ] && JUNIT_TESTCASE="node-network-filter"
[ -z "$ARTIFACT_DIR" ] && ARTIFACT_DIR="/tmp"
TEST_VERSION=$(oc version -o json | jq -r '.openshiftVersion')

python3.11 $krkn_loc/run_kraken.py --config=/tmp/node-network-filter-config.yaml \
-o /tmp/report.out \
